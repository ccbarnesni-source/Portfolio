...
$$
\min_{\boldsymbol{\beta}} f = \min_{\boldsymbol{\beta}} \lVert \mathbf{y} - \mathbf{X}\boldsymbol{\beta} \rVert^2
$$
...
where here we have set $f(\mathbf{X}) = \lVert \mathbf{y} - \mathbf{X}\boldsymbol{\beta} \rVert^2 $. Now setting the gradient to $\mathbf{0}$ we have
...
$$
\nabla^2_{\boldsymbol{\beta}}f = \mathbf{X}^T\mathbf{X}
$$
